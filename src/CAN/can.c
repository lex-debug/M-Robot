...
/* Function Name		: CAN_RxMsg
 * Function Description : This function is called to receive data with bxCAN without intrrupt .
 * Function Remarks		:
 * Function Arguments	: CANx				CANx can be CAN1 or CAN2
 * 						  StdId_11bits		The standard identifier for the CAN frame message that you want to receive .
...
uint8_t CAN_RxMsg(CAN_TypeDef* CANx, uint32_t * StdId, uint8_t * buf, uint8_t len)
{
    ...
    CAN_Receive(CANx, CAN_FIFO0, &CAN1_RxMessage);				//reveive data
    if (*StdId== CAN1_RxMessage.StdId){
    for(i=0;i<len;i++)
		buf[i]=CAN1_RxMessage.Data[i];
    }
    else
    	return 0 ;
	return 1;
