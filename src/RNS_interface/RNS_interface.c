 ...
 * 						  rns 			pointer to a RNS data structure with RNS_interface _t type
 * Function Return		: None
 * Function Example		: float fZoneL1[2][5] = {{0.5, 1.5, 1.5, 0.0,1.5},					//for coordinates use this type of naming
					   	   	   	   	   	   	    {1.0, 3.9, 1.0, 0.0,1.0}}
 * 							RNSOdnGoto(fZoneL1,2,&rns);
 */

void RNSPPstart(float allpoints[][5],int no_point,RNS_interface_t* rns){
	RNSSet(rns, RNS_PPSend_num_Point,(float)no_point);
	int k;
	for(k=0;k<no_point;k++)
		RNSSet(rns, RNS_PPSendPoint, allpoints[k][0],allpoints[k][1],allpoints[k][2],allpoints[k][3],allpoints[k][4]);
